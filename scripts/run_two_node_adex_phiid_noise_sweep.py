#!/usr/bin/env python3
"""Run a 2-node AdEx mean-field dynamical-noise PhiID sweep."""
...
def _build_stub(noise_value: float, seed: int, g_value: float) -> str:
...
def _simulate_single_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    weights = np.array([[0.0, 1.0], [1.0, 0.0]], dtype=float)
    tract_lengths = np.ones((2, 2), dtype=float)
    ...
    if rates.ndim != 2 or rates.shape[1] != 2:
        raise ValueError(f"Expected raw excitatory output with shape (time, 2), got {rates.shape}.")
    time_series = rates.T
...
def _run_simulation_jobs(jobs: list[dict[str, Any]], workers: int) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows).sort_values(["noise_value", "seed"]).reset_index(drop=True)
...
def _write_note(path: Path, args: argparse.Namespace) -> None:
    text = f"""# Two-Node AdEx PhiID Noise Sweep

This sweep uses two symmetrically coupled Zerlaut AdEx mean-field nodes.

Simulation settings:

- fixed coupling `G = {args.g_value}`
...
- `noise_alpha = 0.0`, so the noise is private rather than shared
- `shared_noise_mode = 'none'`

PhiID:

- bivariate PhiID is computed directly on the two excitatory-rate time series
- the resulting atoms are `STS` and `RTR`

Interpretation:

- unlike the simple nonlinear toy model, AdEx mean fields do not guarantee that
  synergy must increase with noise
- this sweep is therefore a biophysical test of whether increased dynamical
  noise can shift the two-node interaction toward stronger synergy
"""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for noise_value in args.noise_values:
        for seed in args.seeds:
            stub = _build_stub(noise_value, seed, args.g_value)
            input_path = input_dir / f"{stub}.mat"
    ...
    if jobs:
        manifest_new = _run_simulation_jobs(jobs, workers=int(args.python_workers))
        if manifest_path.exists():
            manifest_old = pd.read_csv(manifest_path)
            manifest = pd.concat([manifest_old, manifest_new], ignore_index=True)
            manifest = manifest.drop_duplicates(subset=["stub"], keep="last").sort_values(["noise_value", "seed"]).reset_index(drop=True)
        else:
            ...
            [
                {
                    ...
                    "noise_value": float(noise_value),
                    "seed": int(seed),
                    "input_path": str(input_dir / f"{_build_stub(noise_value, seed, args.g_value)}.mat"),
                }
                for noise_value in args.noise_values
                for seed in args.seeds
            ]
    ...
    index_df = load_two_node_phiid_index(phiid_dir, manifest_path=manifest_path)
    expected_outputs = int(manifest.shape[0]) * int(len(args.measures))
    if int(index_df.shape[0]) != expected_outputs:
        raise RuntimeError(
            f"Incomplete two-node PhiID output set: expected {expected_outputs} files, found {int(index_df.shape[0])}."
        )

    raw_df, summary_df = summarize_two_node_outputs(index_df)
    raw_df.to_csv(tables_dir / "raw_atom_values.csv", index=False)
    summary_df.to_csv(tables_dir / "summary_atom_values.csv", index=False)

    fig_pub, _ = plot_two_node_publication(summary_df, g_value=float(args.g_value))
    _save_fig(fig_pub, figures_dir, "two_node_adex_noise_sweep_primary")
    fig_rep, _ = plot_two_node_replicates(raw_df, summary_df, g_value=float(args.g_value))
    _save_fig(fig_rep, figures_dir, "two_node_adex_noise_sweep_replicates")
    fig_sts, _ = plot_two_node_sts_only(summary_df, g_value=float(args.g_value))
    _save_fig(fig_sts, figures_dir, "two_node_adex_noise_sweep_sts_only")
    ...
    summary = {
        ...
        "g_value": float(args.g_value),
        "noise_values": [float(x) for x in sorted(args.noise_values)],
        "seeds": [int(x) for x in args.seeds],
        "measures": [str(x).lower() for x in args.measures],
        "figure_example": str(figures_dir / "two_node_adex_noise_sweep_sts_only.png"),
    }
    (logs_dir / "run_summary.json").write_text(json.dumps(summary, indent=2))
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-root", type=str, default=str(legacy_results("results", "two_node_adex_phiid_noise_sweep")))
    parser.add_argument("--g-value", type=float, default=0.2)
    parser.add_argument(
        "--noise-values",
        type=float,
        nargs="+",
        default=[2.5e-5, 5e-5, 7.5e-5, 1e-4, 1.25e-4, 1.5e-4, 2e-4, 3e-4, 5e-4, 7.5e-4, 1e-3, 2e-3],
    )
    ...
    parser.add_argument(
        "--matlab-runner",
        type=str,
        default=str(_REPO_ROOT / "scripts" / "phiid_two_node_adex_sweep.m"),
    )
    return parser
