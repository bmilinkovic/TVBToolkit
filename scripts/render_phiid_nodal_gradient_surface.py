#!/usr/bin/env python3
"""Render MMI/CCS cohort nodal gradients on a cortical surface."""
...
def _load_gradient_tables(results_root: Path) -> dict[str, pd.DataFrame]:
    out: dict[str, pd.DataFrame] = {}
    for method in METHOD_ORDER:
        path = results_root / method / "tables" / "cohort_nodal_gradients.csv"
        df = pd.read_csv(path)
        out[method] = df
    return out
...
def _project_hemispheres(volume_img: nib.Nifti1Image, fsavg: dict[str, str]) -> tuple[np.ndarray, np.ndarray]:
    left = surface.vol_to_surf(volume_img, fsavg["pial_left"])
    right = surface.vol_to_surf(volume_img, fsavg["pial_right"])
...
def _plot_one_cell(
    ...
    *,
    ...
    # Enlarge the hemispheres slightly without allowing overlap.
...
def render_surface_panel(
    *,
    ...
    gradient_tables = _load_gradient_tables(results_root)
    ...
    for row_idx, method in enumerate(METHOD_ORDER):
        ...
        for col_idx, cohort in enumerate(COHORT_ORDER):
            ...
            if row_idx == 0:
                ...
                y_top = max(pos_l.y1, pos_r.y1) + 0.012
                fig.text(x_mid, y_top, cohort.upper(), ha="center", va="bottom", fontsize=16)
    ...
    # One shared directional annotation for both rows.
    ...
    arrow_ax.add_patch(
        Polygon(
            [[0.5, y_top], [0.18, shaft_y1], [0.82, shaft_y1]],
            closed=True,
            facecolor="#B00020",
            edgecolor="none",
    ...
    arrow_ax.add_patch(
        Polygon(
            [[0.5, y_bottom], [0.18, shaft_y0], [0.82, shaft_y0]],
            closed=True,
            facecolor="#1446A0",
            edgecolor="none",
            transform=arrow_ax.transData,
    ...
    stem = output_dir / "cohort_nodal_gradient_surface_mmi_ccs_2x5_lateral"
