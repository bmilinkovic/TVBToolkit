#!/usr/bin/env python3
"""Run a 2-node AdEx mean-field PhiID sweep across global coupling and noise."""
...
def _write_note(path: Path, args: argparse.Namespace) -> None:
    text = f"""# Two-Node AdEx PhiID G x Noise Sweep

This sweep uses two symmetrically coupled Zerlaut AdEx mean-field nodes.

Simulation settings:

- `20` values of global coupling `G`
- `20` values of dynamical noise `weight_noise`
...
- `G` spans from `{min(args.g_values):g}` to `{max(args.g_values):g}`
- `weight_noise` spans from `{min(args.noise_values):g}` to `{max(args.noise_values):g}`

PhiID:

- bivariate PhiID is computed directly on the two excitatory-rate time series
- the resulting atom values are averaged across seeds for each `(G, noise)` cell
- heatmaps show mean `STS` and `RTR` intensity over the 2D parameter plane
"""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if jobs:
        ...
        if manifest_path.exists():
            ...
            manifest = manifest.drop_duplicates(subset=["stub"], keep="last").sort_values(["g_value", "noise_value", "seed"]).reset_index(drop=True)
        else:
            ...
            [
                {
                    ...
                    "noise_value": float(noise_value),
                    "seed": int(seed),
                    "input_path": str(input_dir / f"{_build_stub(g_value, noise_value, seed)}.mat"),
                }
                for g_value in args.g_values
                for noise_value in args.noise_values
                for seed in args.seeds
    ...
    for measure in [str(x).lower() for x in args.measures]:
        fig, _ = plot_two_node_g_noise_heatmaps(summary_df, measure=measure, g_values=g_values, noise_values=noise_values)
        _save_fig(fig, figures_dir, f"{measure}_g_noise_heatmaps")
    ...
    summary = {
        ...
        "n_g_values": int(len(g_values)),
        "n_noise_values": int(len(noise_values)),
        "seeds": [int(x) for x in args.seeds],
        "measures": [str(x).lower() for x in args.measures],
        "figure_example": str(figures_dir / f"{str(args.measures[0]).lower()}_g_noise_heatmaps.png"),
    }
    (logs_dir / "run_summary.json").write_text(json.dumps(summary, indent=2))
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--python-workers", type=int, default=max(1, os.cpu_count() or 1))
